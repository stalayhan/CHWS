...
/* ####- Bonus Part only doesnt work file has 5 integer.!         */
/* ####- Otherwise works clearly.								  */
/* ----------------------                                         */
/*                                                                */
/*################################################################*/
/*################################################################*/
...
{ 
	...
	int int4Done = FALSE;
	int int5Done = FALSE;
	int result, bonusPartResult, temp;	/* temp process for operator precedence */
	...
	else if (numOfint == 3)
	{
		...
		if (sign1 == '*' || sign1 == '/')
		{
			bonusPartResult = calculate(int1Done, int2Done, sign1);
			bonusPartResult = calculate(bonusPartResult, int3Done, sign2);
		}
		else
		{
			bonusPartResult = calculate(int2Done, int3Done, sign2);
			bonusPartResult = calculate(int1Done, bonusPartResult, sign1);
		}
	}
	else if (numOfint == 4)
	{
		...
		if (sign1 == '*' || sign1 == '/')
		{
			...
			{
				bonusPartResult = calculate(bonusPartResult, int3Done, sign2);
				bonusPartResult = calculate(bonusPartResult, int4Done, sign3);
			}
			else
			{
				temp = calculate(int3Done, int4Done, sign3);
				bonusPartResult = calculate(bonusPartResult, temp, sign2);
			}
		}
		else
		{
			...
			{
				bonusPartResult = calculate(int3Done, int4Done, sign3);
				bonusPartResult = calculate(int2Done, bonusPartResult, sign2);
				bonusPartResult = calculate(int1Done, bonusPartResult, sign1);
			}
		}
	}
	...
	printf("\n______________RESULT_____________\n");
	printf("Result is :%5d\n",result);
	printf("BonusPart Result is: %5d\n",bonusPartResult );
